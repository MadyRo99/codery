    ...
    <div class="uploadBox">
        ...
        <form role="form" enctype="multipart/form-data" @submit.prevent="onSubmit">
            ...
            <div class="uploadBoxMain" v-else>
                ...
                <button @click="removeItems" class="btn btn-danger btn-round">{{removeFileMessage}}</button>
                ...
                axios({method: this.method, url: this.postURL, data: this.formData,headers:this.postHeader})
                    .then((response) => {
                        ...
                        if (response.data.success) {
                            ...
                            this.$emit('upload-error', response.data.info);
                        }
                    })
                    .catch((error) => {
                        this.isLoaderVisible = false;
                        ...
                        this.removeItems();
                        this.$emit('upload-error', "Se pare ca a aparut o problema la incarcarea imaginilor. Te rog incearca din nou mai tarziu.");
                    });
            } else {
                if(this.showHttpMessages)
